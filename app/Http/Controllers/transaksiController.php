...
class transaksiController extends Controller
{
    ...
    public function searchBarang(Request $request)
    {
        if($request->ajax()) {
            ...
            $data = Barang::where('namaBarang', 'LIKE', $request->namaBarang.'%')
                ->get();
            ...
            if (count($data)>0) {
                foreach ($data as $b){
                    ...
                    <div class="card" style="width: 15rem; margin-left: 25px; margin-right: 10px">
                        <form action="'.route('tr.add', $b->id).'" method="POST">


                        </form>
                    </div>
                    ';
                    '<a class="btn btn-xs btn-success" href="'.route('barang.view', $data->id).'">Lihat</a>'
                }
            } else {
                echo 'asd';
            }
